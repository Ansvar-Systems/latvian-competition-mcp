# ─────────────────────────────────────────────────────────────────────────────
# Latvian Competition MCP — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t latvian-competition-mcp .
# Run:    docker run --rm -p 3000:3000 latvian-competition-mcp
#
# The image bakes /app/data/cc-lv.db at build time. Override CC_LV_DB_PATH
...
ENV CC_LV_DB_PATH=/app/data/cc-lv.db

# Copy node_modules (with the rebuilt .node binding) from the builder.
# Do NOT run `npm ci` here — the runtime stage has no python3/make/g++ so
# the rebuild would fail and we'd ship a binding-less image again.
COPY --from=builder /app/node_modules ./node_modules
COPY --from=builder /app/dist/ dist/
COPY package.json package-lock.json* ./

# Bake the pre-built database into the image so /app/data/cc-lv.db resolves
...
# We then COPY it into the image at /app/data/cc-lv.db (CC_LV_DB_PATH). The
# explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match.
COPY data/database.db data/cc-lv.db

# Ship ingestion / seed scripts so operators can re-build the DB inside the
# container if needed.
